...
class PopController extends BaseController {
     ...
     * )
     */
    public function createTicket($type, $expireSeconds, $sceneId){
        $token = self::getToken();
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.$token;
        $queryAction = 'POST';
        $template = array();
        if($type == 1){
        ...
        return A('Home/Curl')->callWebServer($queryUrl, $template, $queryAction);
    }
     ...
     */
    public function getQrcode($ticket,$act_id){
        $queryUrl = 'https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.urlencode($ticket);
        $queryAction = 'GET';
        $result = A('Curl')->callWebServer($queryUrl, '', $queryAction, 0);
         $filename='/var/www/html/sign/Public/qrcode/'."{$act_id}".'.jpg';
    ...
    public function long2short($longUrl){
        $token = self::getToken();
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/shorturl?access_token='.$token;
        $queryAction = 'POST';
        $template = array();
        $template['long_url'] = $longUrl;
        $template['action'] = 'long2short';
        return A('Curl')->callWebServer($queryUrl, '', $queryAction);
    }
